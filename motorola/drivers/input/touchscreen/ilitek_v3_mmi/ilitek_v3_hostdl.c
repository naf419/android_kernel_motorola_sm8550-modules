...
static int ilitek_tddi_fw_iram_upgrade(u8 *pfw, bool mcu)
{
	...
	u32 mode, crc, dma, iram_crc = 0, slave_crc;
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		if (fbi[i].mode == mode && fbi[i].len != 0) {
			...
			if (ilits->cascade_info_block.nNum != 0) {
				ili_mspi_read_slave_register(DMA_CRC_ADDR, &slave_crc, sizeof(u32));
				...
				ILI_INFO("Slave %s CRC is %s hex(%x) : dma(%x) : iram(%x)\n",
					fbi[i].name, ((slave_crc != dma) || (iram_crc_err)) ? "Invalid !" : "Correct !", slave_crc, dma, iram_crc);

				if ((crc != dma) || (slave_crc != dma) || iram_crc_err) {
